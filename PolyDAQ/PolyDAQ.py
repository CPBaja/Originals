...
#        calibrate.triggered.connect (self.calibrateBoard)
        ...
        self.addNote = toolsMenu.addAction ('Earmark Data File...')
...
#        self.addNote.setEnabled (False)
        ...
        self.dontShowFileSavedDialogAgain = False
...
#            self.histogram_btn.setEnabled (True)
    ...
    def change_file_name (self, FileName_txt):
...
#
        ...
        FileName_txt.clearFocus()
        self.file_name = FileName_txt.text()
        
		# does filename have .csv extension?  if not, add it!
        if self.file_name[-4:] != '.csv':
            self.file_name += '.csv'

        if self.old_file_name <> self.file_name:
...
#                self.cancelClicked = True
    ...
    def reset_file_name (self):
        ...
        self.file_name = time.strftime("%a_%m.%d.%y_%I.%M%p.csv")
...
#        PyQt4.QtGui.QToolTip.setFont (PyQt4.QtGui.QFont ('Courier', 12, PyQt4.QtGui.QFont.Bold))
...
#                      self.change_file_name)
###########  TEST CODE FOR NEW FILE NAME INPUT
        self.FileName_lbl = PyQt4.QtGui.QLabel ("Data File:")  
#        self.old_name = self.file_name        
        self.FileName_txt = PyQt4.QtGui.QLineEdit ()
        self.FileName_txt.setText(self.file_name)
        self.FileName_txt.setFixedWidth (190)
        self.FileName_txt.editingFinished.connect (functools.partial(self.change_file_name, self.FileName_txt ) )
        self.resetFileName_btn = PyQt4.QtGui.QPushButton (u'\u23f0')
        self.resetFileName_btn.setFont(PyQt4.QtGui.QFont ('Courier', 22 , 
#                                                           PyQt4.QtGui.QFont.Cursive, 
#                                                            PyQt4.QtGui.QFont.Bold 
                                                            ))
        self.resetFileName_btn.setFixedWidth (20)
        self.resetFileName_btn.setFixedHeight (20)

        self.resetFileName_btn.setToolTip (" Reset File Name to Current Date & Time ")

        self.connect (self.resetFileName_btn,
                      PyQt4.QtCore.SIGNAL ("clicked()"),
        ...
        layout.addWidget (self.resetFileName_btn,  2, 3)
    ...
    def ch_name_edit_finished (self, a_channel):
        ...
        '''
        a_channel['edit'].clearFocus()  #clears focus on QLineEdit specifically if enter is pressed
...
#        self.scan_rate_info_btn.setStyleSheet("background-color: #eedd82") #yellowish
        self.scan_rate_info_btn.setStyleSheet("QPushButton {    \
                                                            background-color: #007cc3; \
                                                            color: white\
                                                            }") #this changes just the pushbuton 
                                                                #color (to blue) and the font color (to white)
...
#                      self.auto_or_manual_stop)
    ...
    def createStartStopGroup (self):
        self.start_stop_grp = PyQt4.QtGui.QGroupBox ('Record/Stop')

        self.start_btn = PyQt4.QtGui.QPushButton ('RECORD')
        self.start_btn.setFixedWidth (100)
        self.start_btn.setFixedHeight (40)
        self.start_btn.setStyleSheet("QPushButton {background-color: green; color: black}")
        self.start_btn.setFont (PyQt4.QtGui.QFont ('SansSerif', 14))
        self.start_btn.setToolTip(' Start recording & saving data ')

        
        self.stop_btn = PyQt4.QtGui.QPushButton ('STOP&&SAVE')
        ...
        self.stop_btn.setStyleSheet("QPushButton {background-color: Lightgrey; color: gray}")
...
#New from Plot Manager:
                ...
                self.start_btn.setStyleSheet("QPushButton {background-color: Lightgrey; color: gray}")
                self.start_btn.setEnabled (False)
                self.start_btn.update ()
                
                self.stop_btn.setStyleSheet("QPushButton {background-color: red; color: black}")
...
#New from Plot Manager
            ...
            #This "tries" to close the earmark window if it was left open
            try:
            ...
            #Tell them they just saved the data!  Unless they already turned off this dialog.            
            if self.dontShowFileSavedDialogAgain == False: 
                
                self.fileSavedDialog()
    #----------------------------------------------------------------------------------

    def fileSavedDialog (self):

        self.fileSavedDialogScreen = PyQt4.QtGui.QDialog (self)

        fileSavedDialogScreen_lbl = PyQt4.QtGui.QLabel (
                                'Your data have been saved to the file:\n\n          '+
                                              self.file_name +
                                              '\n\nLocated in the folder:\n\n          ' 
                                              +self.file_path +
                                              '\n\n(Look for the folder on the desktop!)\n\n')
        
        dontShowAgain_checkbox = PyQt4.QtGui.QCheckBox ("Don't show this message again")
        
        dontShowAgain_checkbox.clicked.connect (self.dontShowWasClicked)
        
        dialogClose_btn = PyQt4.QtGui.QPushButton ("OK")
        dialogClose_btn.setFixedWidth (80)
        
        dialogClose_btn.clicked.connect (self.fileSavedDialogClose)              
        
        fileSavedDialogScreen_Layout = PyQt4.QtGui.QGridLayout ()        

        fileSavedDialogScreen_Layout.addWidget (fileSavedDialogScreen_lbl, 0,0,1,3)        
        fileSavedDialogScreen_Layout.addWidget (dontShowAgain_checkbox, 1,0)
        fileSavedDialogScreen_Layout.addWidget (dialogClose_btn, 2,1)



        
        self.fileSavedDialogScreen.setLayout (fileSavedDialogScreen_Layout)
    
        self.fileSavedDialogScreen.setWindowTitle ('File Saved!')
        self.fileSavedDialogScreen.setWindowIcon (PyQt4.QtGui.QIcon ('polydaq_icon.png'))

        self.fileSavedDialogScreen.show ()

    def dontShowWasClicked (self):
        print ('here too')        
        self.dontShowFileSavedDialogAgain = True

    def fileSavedDialogClose (self):
        self.fileSavedDialogScreen.close ()
...
####### The next 4 lines prevent opening more than one window!  The code "tries" to 
####### close the earmark screen first.  If no earmark screen exists, NO BIGGIE, just "pass."
        try:
            self.earmarkScreen.close()
        except:
            pass
...
#        self.addNote.setEnabled(False)   # disable the menu item, or else the Qdialog can be opened again!
        ...
        earmarkScreenLayout = PyQt4.QtGui.QGridLayout ()
...
#        blankLine = PyQt4.QtGui.QLabel (' ')
        ...
        earmarkScreenLayout.addWidget (earmarkScreenText, 0,0,1,3)
        earmarkScreenLayout.addWidget (self.earmarkLabel, 1,0)        
        earmarkScreenLayout.addWidget (self.earmark, 1,1)
        earmarkScreenLayout.addWidget (self.addEarmark_btn,1,2)


        earmarkScreenLayout.addWidget (self.earmarkStatus,2,1)
#        earmarkScreenLayout.addWidget (blankLine)
        earmarkScreenLayout.addWidget (okButton,4,2)
